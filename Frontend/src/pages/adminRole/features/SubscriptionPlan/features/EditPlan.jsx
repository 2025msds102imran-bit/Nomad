...
export default function EditPlan({ plan, onBack, onUpdate }) {
  const [name, setName] = useState(plan?.name ?? "");
  const [userType, setUserType] = useState(plan?.type ?? "");
  const [durationDays, setDurationDays] = useState(plan?.durationDays ?? "");
  const [status, setStatus] = useState(plan?.status ?? "");
  const [price, setPrice] = useState(plan?.rawPrice ?? "");
  const [currency, setCurrency] = useState(plan?.currency ?? "");
  const [description, setDescription] = useState(plan?.description ?? "");

  useEffect(() => {
    setName(plan?.name ?? "");
    setUserType(plan?.type ?? "");
    setDurationDays(plan?.durationDays ?? "");
    setStatus(plan?.status ?? "");
    setPrice(plan?.rawPrice ?? "");
      ...
      >
        <div className="w-full h-20 p-3.5 bg-gradient-to-b from-slate-900 to-blue-900 flex items-center">
          ...
          <div className="ml-3 flex flex-col">
            <div className="text-white text-lg font-semibold font-['Inter'] leading-8">Edit Subscription Plan</div>
            <div className="text-white/80 text-xs font-normal font-['Inter'] leading-5">Update plan details and pricing</div>
          </div>
        </div>
